...
codec:
	$(ES) $(PWD)/script/codec.erl gaby $(PWD)/proto/ $(PWD)/src
...
console-dev:
	_build/dev/rel/gaby/bin/gaby console

rel-prod:
	$(SED) -i 's/{gaby, "gaby-version"}/{gaby, "$(VER)"}/g' ./rebar.config
	$(PWD)/script/rebar3 as prod release
	$(PWD)/script/rebar3 as prod tar
	$(SED) -i 's/{gaby, "$(VER)"}/{gaby, "gaby-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/prod/rel/gaby/gaby-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/prod/rel/gaby/gaby-$(VER).tar.gz
...
rel-stage:
	...
	$(SED) -i 's/{gaby, "$(VER)"}/{gaby, "gaby-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/stage/rel/gaby/gaby-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/stage/rel/gaby/gaby-$(VER).tar.gz
